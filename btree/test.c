...
TEST(test_tree_delete_right_subtree, "Delete a node with only right subtree (X)")
bst_init(&test_tree);
bst_insert_many(&test_tree, base_keys, base_values, base_data_count);
bst_insert_many(&test_tree, additional_keys, additional_values,
                additional_data_count);
...
TEST(test_tree_find_deep, "Find P node")
bst_init(&test_tree);
bst_insert_many(&test_tree, base_keys, base_values, base_data_count);
bst_insert_many(&test_tree, additional_keys, additional_values,
                additional_data_count);
...
TEST(test_balanceMy, "Muj balance");
bst_init(&test_tree);
const int muj_count = 15;
const char muj_keys[] = {'A', 'B', 'B', 'B', 'B', 'J', 'N', 'A',
                          'C', 'E', 'G', 'I', 'K', 'M', 'O'};
const int muj_values[] = {8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 16};
bst_insert_many(&test_tree, muj_keys, muj_values, muj_count);
bst_balance(&test_tree);
bst_print_tree(test_tree);
ENDTEST
